...
export default async function DealHealthPage({ searchParams }: { searchParams: Promise<{ kind?: string }> }) {
  ...
  return <div><PageHeader eyebrow="Commercial intelligence" title="Deal Health" description="Explainable alerts derived from live quotation, discount, inventory, and delivery data."/><HealthSummaryCards counts={data.counts} active={kind}/><section className="panel section-gap"><div className="panel-heading"><div><span className="eyebrow">Live risk register</span><h2>{kind ? kind.replaceAll("_", " ").toLowerCase() : "All alerts"}</h2></div></div><AlertTable alerts={visible} canAct={canAct} emptyDescription={`Thresholds: stale after ${data.policy.staleAfterDays} days, anomaly +${data.policy.anomalyDeltaBps / 100} pts — edit in Settings → Policy.`}/></section></div>;
}
