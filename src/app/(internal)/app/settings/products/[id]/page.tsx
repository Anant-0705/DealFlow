...
export default async function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [product, categories, plans, warehouses] = await Promise.all([
    getProduct(Number(id)),
    prisma.category.findMany({ orderBy: { name: "asc" } }),
    listPlans(),
    prisma.warehouse.findMany({ where: { active: true }, orderBy: { name: "asc" }, select: { id: true, name: true } }),
  ]);
  if (!product) notFound();
  return <div><Link href="/app/settings/products" className="back-link">← Products</Link><div className="settings-grid">
    <ProductForm categories={categories} plans={plans} warehouses={warehouses} product={product}/>
