...
export async function getDashboardData(session: AppSession) {
  ...
  const [pendingApprovals, openQuotations, awaitingFulfillment, unpaidInvoices, revenue, recentActivity, tasks, health] = await Promise.all([
    ...
    prisma.invoice.findMany({ where: { status: { in: ["UNPAID", "PARTIAL"] }, order: { quote: ownedQuote } }, select: { totalPaise: true, paidPaise: true } }),
  ...
  return {
    metrics: {
      ...
      unpaidBalancePaise: unpaidInvoices.reduce((sum, invoice) => sum + invoice.totalPaise - invoice.paidPaise, 0),
