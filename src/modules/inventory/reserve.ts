...
export async function reserveSuggestedAllocations(db: Prisma.TransactionClient, orderId: number) {
  ...
  const order = await db.order.findUniqueOrThrow({
    where: { id: orderId },
    include: { lines: { include: { product: { include: { category: true } }, variant: true, allocations: true } } },
  });
  ...
  const [stock, warehouses] = await Promise.all([
    ...
    db.warehouse.findMany({ where: { active: true }, select: { id: true, name: true, shippingCostWeightPaise: true } }),
  ]);
