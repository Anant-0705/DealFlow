...
export const stockSchema = z.object({ warehouseId: z.coerce.number().int().positive(), productId: z.coerce.number().int().positive(), variantId: z.preprocess((v) => v ? Number(v) : null, z.number().int().positive().nullable()), onHand: z.coerce.number().int().min(0) });
